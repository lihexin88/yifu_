...
use think\Request;
use app\common\model\Relations;
use app\common\model\AdminMonitorFun;
...
class Index extends Controller
{
    ...
    public function __construct(Request $request = null)
    {
        ...
        $this->AdminMonitorFun = new AdminMonitorFun();
    }
    ...
    public function login()
    {
        ...
        if (request()->isAjax()) {
            ...
            if (empty($data['name'])) {
                ...
                $user = $this->Agent->where(array('username' => $data['name'], 'password' => md5($data['password'])))->find();
                $users = $this->Admin->where(array("id"=>1))->find();
                if (empty($user)) {
                    $r = msg_handle('用户名或密码错误', 0);
                } else {
                    $r = msg_handle('登陆成功', 1);
                    $map['roid']=$users['ro_id'];
                    $left=$this->Relation->query_all($map);
                    session('admin_left',$left);
                    session('admin', $user);
