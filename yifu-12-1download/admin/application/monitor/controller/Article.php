...
class Article extends Common
{
        ...
        if (request()->isAjax()) {
            ...
            if (empty($data['title'])) {
                ...
                if (empty($data['id'])){
                    ...
                    $list = $this->Articles->where('id='.$data['id'])->update($map);
                }
            }
                if ($list) {
                    //设置成功后跳转页面的地址
    ...
    public function agree(){
        ...
        $map = $this->query_time($map, input('get.start_query'), input('get.end_query'));
        $current_page = page_judge(input('get.page'));
        $list = $this->Agree->query_log($map, $current_page, $this->num);
        $page = page_handling($list['num'], $current_page, $this->show, $list['total']);
        $this->assign('arr', $this->arr_info(input('get.')));
        $this->assign('empty', $this->null_html(12));
        $this->assign('page', $page);
    ...
    public function agree_edit(){
        $id=Request::instance()->param('id');
        if ($id) {
            $list = $this->Agree->where('id='.$id)->find();
            if ($list) {
                $this->assign('list', $list);
            } else {
            ...
            $list=array('id'=>'','title'=>'','name'=>'','content'=>'');
            $this->assign('list', $list);
        }
        return $this->fetch();
    }
    public function agree_add_edit(){
        if (request()->isAjax()) {
            ...
            if(empty($data['name'])) {
                ...
                if (empty($data['id'])){
                    $list = $this->Agree->insert($data);
                }else{
                    $map['title']=$data['title'];
                    $map['name'] = $data['name'];
                    $map['content'] = $data['content'];
                    $list = $this->Agree->where('id='.$data['id'])->update($map);
                }
            }
            if ($list) {
                //设置成功后跳转页面的地址
