...
class TradeBuy extends Common
{
    ...
    public function index($id, $contract, $pattern, $direction, $mode, $classify, $price, $number)
    {
        ...
        $total = $number * ($stock['bond'] + $stock['fee']);
        if ($total > $account['account']) {
            ...
            if (empty($price)) {
                ...
                $r = $this->data_handle($id, $contract, $pattern, $direction, $mode, $classify, $price, $number, $account, $stock);
            }
        }
        return $r;
    }

    public function data_handle($id, $contract, $pattern, $direction, $mode, $classify, $price, $number, $account, $stock)
    {
        $bond = $number * $stock['bond'];
        ...
        $res2 = $this->CapitalFlow->add_log($id, $bond, $balance, 2, $account);
        $res3 = $this->CapitalFlow->add_log($id, $fee, $balance1, 3, $account);
        $res4 = $this->Entrust->add_log($id, $contract, $number, $price, $fee, $bond, $mode, $classify, $direction, $pattern, $stock, $account);
