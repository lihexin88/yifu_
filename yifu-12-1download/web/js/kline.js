...
DarkTheme.prototype.__construct = function () {
    ...
    if (KlineIns.reverseColor) {
    	...
    	//K线颜色
        this._colors[Theme.Color.Positive] = "#ff3c3c";//涨的颜色
        this._colors[Theme.Color.Negative] = "#00f0f0";//跌的颜色
    ...
    this._colors[Theme.Color.Background] = "#000";//背景色
    ...
    this._colors[Theme.Color.Indicator2] = "#f600ff";//MACD 字体颜色
    ...
    this._colors[Theme.Color.Grid0] = "#be140a";//x轴虚线
    this._colors[Theme.Color.Grid1] = "#be140a";//xy轴线
    this._colors[Theme.Color.Grid2] = "#666";
    this._colors[Theme.Color.Grid3] = "#fff";//x apoint 边框颜色
    this._colors[Theme.Color.Grid4] = "#fff";//y apoint 边框颜色
    this._colors[Theme.Color.TextPositive] = "#ff3c3c";//头部涨幅大于0的颜色
    this._colors[Theme.Color.TextNegative] = "#00f0f0";//头部涨幅小于0的颜色
    this._colors[Theme.Color.Text0] = "#444";//
    this._colors[Theme.Color.Text1] = "blue";
    this._colors[Theme.Color.Text2] = "#f0f0f0";//y轴刻度字体颜色
    this._colors[Theme.Color.Text3] = "#f0f0f0";//Y轴apoint字体颜色
    this._colors[Theme.Color.Text4] = "#f0f0f0";//X轴apoint字体颜色//头部:时间开高低收字体颜色
        ...
        $(".chart_container .chart_toolbar_tabgroup a")
            .click(function () {
            	var type = $(this).parent().attr('name');
              	if(type == "5m") {
              		change_url(KlineIns.symbol, "getFewMinLine", "type=", "5");	
				} else if(type == "15m") {
					change_url(KlineIns.symbol, "getFewMinLine", "type=", "15");	
				} else if(type == "30m") {
					change_url(KlineIns.symbol, "getFewMinLine", "type=", "30");
				} else if(type == "1h") {
					change_url(KlineIns.symbol, "getFewMinLine", "type=", "60");
				} else if(type == "1d") {
					change_url(KlineIns.symbol, "getDailyKLine","","1d");
				} else if(type == "1w") {
					change_url(KlineIns.symbol, "getDailyKLine","","1w");
				} else if(type == "line"){
					change_url(KlineIns.symbol, "getFewMinLine", "type=", "5");	
				}
...
//          },
            success: function (res) {
                if (KlineIns.G_HTTP_REQUEST) {
                	if(KlineIns.limit != "1w"){
                		var elements = eval("_" + KlineIns.symbol + "_");
						var res = {
							"success": true,
							"data": {
								"lines": []
						...
						for(var i = 0; i < elements.length; i++) {
							var date = new Date(elements[i].d);
							var t = Date.parse(date); //时间
							var o = Number(elements[i].o); //开盘价
							var h = Number(elements[i].h); //最高价
							var l = Number(elements[i].l); //最低价
							var c = Number(elements[i].c); //收盘价
							var v = Number(elements[i].v); //成交量
							var arr = [t, o, h, l, c, v];
							res.data.lines.push(arr);
						}
                	}
					if(KlineIns.limit == "1w"){
						var elements = eval("_" + KlineIns.symbol + "_");
						...
						var res_data_lines = res.data.lines;
						var week_arr = [];//有几条数据
						for(var i = 0;i<res_data_lines.length;i++){
							var time, week, checkDate = new Date(new Date());                              
							checkDate.setDate(checkDate.getDate() + 4 - (checkDate.getDay() || 7));                              
							time = res_data_lines[i][0];                              
							checkDate.setMonth(0);                              
							checkDate.setDate(1);                              
							week = Math.floor(Math.round((time - checkDate) / 86400000) / 7) + 1;
							res_data_lines[i].push(week);
						...
						for(var i = 0;i<week_arr.length;i++){
							var data_account = [];
							for(var j = 0;j<res_data_lines.length;j++){
								if(week_arr[i] == res_data_lines[j][6]){
									data_account.push(res_data_lines[j]);
								}
							}
							var name = "str" + week_arr[i];
						...
						for(var i = 0;i<data_arr.length;i++){
							var arr = [];
							var t = data_arr[i][data_arr[i].length-1][0];
							var o = data_arr[i][0][1];
							var h = data_arr[i][0][2];
							var l = data_arr[i][0][3];
							var c = data_arr[i][data_arr[i].length-1][4];
							var v=0;
							for(var j = 0;j<data_arr[i].length;j++){
								h = h > data_arr[i][j][2]?h:data_arr[i][j][2];
								l = l < data_arr[i][j][3]?l:data_arr[i][j][3];
								v += data_arr[i][j][5];
							}
							arr = [t, o, h, l, c, v];
							line_arr.push(arr);
						}
						res.data.lines = line_arr;
					}
